...
const contractAddress = "0x416cacAa5F9D98Dd0321ba5Bd21a89d648Ae0c2b";

//onst sepoliaWallet3 = "0x416cacAa5F9D98Dd0321ba5Bd21a89d648Ae0c2b";

//var donationcount=1;
var donationcount3 = parseInt(localStorage.getItem("donationcount3"))||2;
let provider;
let contract;
function updateDonationCount3(count) {
  donationcount3 = count;
  localStorage.setItem("donationcount3", count.toString());
}
...
export async function connectWalletAndDonate() {
    if (typeof window.ethereum !== "undefined") {
      try {
        ...
        updateDonationCount3(donationcount3 + 1);
        //donationcount++;
        console.log("bağiş sayisi:",donationcount3);
...
window.addEventListener("load", () => {
  ...
  const storedCount = localStorage.getItem("donationcount3");
  if (storedCount !== null) {
    donationcount3 = parseInt(storedCount);
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
    document.getElementById("donateButton3").addEventListener("click", connectWalletAndDonate);
});

/* ******************************************************************progress bar *************************************************/

async function totalDonate(){
  var total=donationcount3*0.01;
...
setInterval(function() {
  updateProgressFromBlockchain(1, 'c3');
}, 600);
