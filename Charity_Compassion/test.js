...
//var donationcount=3;
var donationcount = parseInt(localStorage.getItem("donationcount"))||0;
let provider;
let contract;

function updateDonationCount(count) {
    donationcount = count;
    localStorage.setItem("donationcount", count.toString());
  }
...
export async function connectWalletAndDonate() {
    if (typeof window.ethereum !== "undefined") {
      try {
        ...
        //donationcount++;
        updateDonationCount(donationcount + 1);
...
window.addEventListener("load", () => {
    ...
    if (storedCount !== null) {
      donationcount = parseInt(storedCount);
    }
  });
